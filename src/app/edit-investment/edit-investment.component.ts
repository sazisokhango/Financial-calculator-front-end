...
export class EditInvestmentComponent implements OnInit {
    ...
    this.investmentService.getById(this.forecastId).subscribe({
      next: forecast => {
        this.form.patchValue({
          ...
          initialAmount:       forecast.initialAmount,
          monthlyContribution: forecast.monthlyContribution,
          termMonths:          forecast.termMonths,
          annualInterestRate:  forecast.annualInterestRate
        });
  ...
  onSubmit(): void {
    ...
    this.investmentService.update(this.forecastId, payload).subscribe({
  ...
  cancel(): void {
    this.router.navigate(['/user', this.userId, 'investments', this.forecastId]);
  }
}
